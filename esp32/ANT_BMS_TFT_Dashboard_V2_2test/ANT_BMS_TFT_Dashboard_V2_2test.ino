...
static const char *FIRMWARE_VERSION = "v2_2test";
...
static void drawStateChip(const char *state, uint16_t accent) {
  resetFont();
  const int16_t chipW = strlen(state) > 20 ? 224 : 150;
  const int16_t chipX = (SCREEN_W - chipW) / 2;
  tft.fillRoundRect(chipX, 24, chipW, 18, 6, rgb(10, 25, 35));
  tft.drawRoundRect(chipX, 24, chipW, 18, 6, accent);
  tft.setTextColor(accent);
  tft.setTextSize(1);
  tft.setCursor(chipX + 8, 30);
  tft.print(state);
}
...
static void drawTryStrongestBmsScreen() {
  drawStateShell("连接最强 BMS", C_GREEN(), true);
  drawStateHint("v2_2test 临时逻辑", "扫描后自动连接");
  drawAsciiCentered("Select strongest RSSI", 145, C_DIM());
}
...
static bool connectStrongestBmsAtBoot(bool reportToPhone) {
  setAppState(TRY_CONNECT_STRONGEST_BMS, "v2_2test boot scans strongest BMS");
  BmsCandidate candidate = scanForBms("");

  if (!candidate.found) {
    NimBLEDevice::getScan()->clearResults();
    Serial.println("[POLICY-TEST] No ANT/BMS candidate found; waiting for mini program config");
  ...
  Serial.printf("[POLICY-TEST] Boot selected strongest BMS: name=\"%s\" mac=%s rssi=%d\r\n",
                candidate.name.c_str(), candidate.mac.c_str(), candidate.rssi);

  if (!connectToBms()) {
    NimBLEDevice::getScan()->clearResults();
    Serial.printf("[POLICY-TEST] Strongest BMS %s connect failed\r\n", candidate.mac.c_str());
  ...
  selectedBmsName = connectedName;
  saveSelectedBms();
  Serial.printf("[POLICY-TEST] Strongest BMS saved as current boot target: %s\r\n",
                selectedBmsMac.c_str());
...
void setup() {
  ...
  Serial.println();
  Serial.printf("=== ANT BMS TFT Dashboard %s Auto Strongest Boot ===\r\n", FIRMWARE_VERSION);
  ...
  Serial.println("[POLICY-TEST] v2_2test boot auto-connects the strongest ANT/BMS candidate");
  if (selectedBmsMac.length() > 0) {
    Serial.printf("[POLICY-TEST] Existing saved BMS %s is kept until a strongest BMS connects successfully\r\n",
                  selectedBmsMac.c_str());
  }
