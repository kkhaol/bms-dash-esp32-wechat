...
static const char *FIRMWARE_VERSION = "ANT_BMS_TFT_Dashboard_V2_4_3_TftBuildConfig";
...
static void displayTest() {
  Serial.println("[DISPLAY] displayTest begin");
  drawDirectTextScreen("X DASH", "DISPLAY OK");
  Serial.println("[DISPLAY] displayTest hold 2000ms");
  delay(2000);
  Serial.println("[DISPLAY] displayTest end");
}
...
static void drawBootAnimation() {
  if (!ensureFrame()) {
    drawDirectTextScreen("X DASH", "STARTING");
    delay(1000);
    return;
  }
...
static void drawStateScreen(const char *title, const char *subtitle, uint16_t accent, bool showPulse) {
  if (!ensureFrame()) {
    ...
    (void) accent;
    (void) showPulse;
    drawDirectTextScreen("CONNECTING", "BATTERY");
    screenReady = true;
    return;
  }
...
static void drawDashboardStatic() {
  if (!ensureFrame()) {
    drawDirectTextScreen("DASHBOARD", "NO SPRITE");
