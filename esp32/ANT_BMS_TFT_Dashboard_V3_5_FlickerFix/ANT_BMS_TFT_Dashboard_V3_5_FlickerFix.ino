...
static char dashboardCachedPower[12] = "";
...
static uint16_t C_DIVIDER() { return rgb(42, 58, 70); }
static uint16_t C_DELTA_VALUE() { return rgb(128, 239, 222); }
static uint16_t C_POWER_VALUE() { return rgb(0, 180, 255); }
static uint16_t C_VOLTAGE_VALUE() { return rgb(255, 198, 70); }
static uint16_t C_CURRENT_VALUE() { return rgb(82, 221, 229); }
...
static int16_t tinyUnitWidth(const char *text) {
  return int16_t(strlen(text) * 6);
}
...
static void drawTinyUnit(TFT_eSprite &canvas, const char *text, int16_t x, int16_t y,
                         uint16_t color) {
  canvas.setTextDatum(TL_DATUM);
...
static const char UI_TXT_RECONNECTING_DOTS[] = "\u6b63\u5728\u91cd\u65b0\u8fde\u63a5...";
...
static const int16_t TOP_TEMP_X = 0;
static const int16_t TOP_TEMP_Y = 0;
static const int16_t TOP_TEMP_W = 105;
static const int16_t TOP_TEMP_H = 32;
static const int16_t TOP_ICON_X = 240;
static const int16_t TOP_ICON_Y = 0;
static const int16_t TOP_ICON_W = 80;
static const int16_t TOP_ICON_H = 32;
static const int16_t LEFT_X = 8;
static const int16_t LEFT_Y = 43;
static const int16_t LEFT_W = 102;
static const int16_t LEFT_H = 104;
static const int16_t SOC_CX = 160;
static const int16_t SOC_CY = 78;
static const int16_t SOC_R = 39;
static const int16_t SOC_DOT_R = 4;
static const int16_t CENTER_X = 110;
static const int16_t CENTER_Y = 29;
static const int16_t CENTER_W = 100;
static const int16_t CENTER_H = 136;
static const int16_t RIGHT_X = 210;
static const int16_t RIGHT_Y = 43;
static const int16_t RIGHT_W = 102;
static const int16_t RIGHT_H = 104;
...
static void drawTopTempArea(TFT_eSprite &canvas, const DashboardUiValues &values,
                            int16_t originX = 0, int16_t originY = 0) {
  canvas.fillRect(TOP_TEMP_X - originX, TOP_TEMP_Y - originY, TOP_TEMP_W, TOP_TEMP_H, C_BG());
  drawTempValue(canvas, values, originX, originY);
}

static void drawTopIconArea(TFT_eSprite &canvas, int16_t originX = 0, int16_t originY = 0) {
  canvas.fillRect(TOP_ICON_X - originX, TOP_ICON_Y - originY, TOP_ICON_W, TOP_ICON_H, C_BG());
  drawConnectionIcons(canvas, originX, originY);
}

static void drawTopBar(TFT_eSprite &canvas, const DashboardUiValues &values,
                       int16_t originX = 0, int16_t originY = 0) {
  drawTopTempArea(canvas, values, originX, originY);
  drawTopIconArea(canvas, originX, originY);
}

static void drawCompactNumberUnitRight(TFT_eSprite &canvas, const char *number, const char *unit,
                                       int16_t rightX, int16_t y, int16_t maxWidth,
                                       const GFXfont *preferredFont, uint16_t numberColor) {
  const GFXfont *numberFont = preferredFont;
  int16_t numberW = freeTextWidth(canvas, number, numberFont);
  const int16_t unitW = unit[0] ? tinyUnitWidth(unit) + 4 : 0;
  if (numberW + unitW > maxWidth) {
    numberFont = &FreeSansBold9pt7b;
    numberW = freeTextWidth(canvas, number, numberFont);
  }

  const int16_t totalW = numberW + unitW;
  const int16_t x = rightX - totalW;
  drawFreeText(canvas, number, x, y, numberFont, numberColor);
  if (unit[0]) {
    const int16_t unitY = numberFont == preferredFont ? y + 18 : y + 12;
    drawTinyUnit(canvas, unit, x + numberW + 4, unitY, C_MUTED());
  }
}

static void drawPowerIcon(TFT_eSprite &canvas, int16_t x, int16_t y, uint16_t color) {
  const int16_t ptsX[6] = {x + 9, x + 2, x + 8, x + 5, x + 15, x + 10};
  const int16_t ptsY[6] = {y, y + 14, y + 14, y + 27, y + 9, y + 9};
  for (uint8_t i = 0; i < 5; i++) {
    canvas.drawLine(ptsX[i], ptsY[i], ptsX[i + 1], ptsY[i + 1], color);
  }
}

static void drawDeltaIcon(TFT_eSprite &canvas, int16_t x, int16_t y, uint16_t color) {
  canvas.drawLine(x, y + 14, x + 6, y + 14, color);
  canvas.drawLine(x + 6, y + 14, x + 10, y + 5, color);
  canvas.drawLine(x + 10, y + 5, x + 15, y + 24, color);
  canvas.drawLine(x + 15, y + 24, x + 21, y + 14, color);
  canvas.drawLine(x + 21, y + 14, x + 27, y + 14, color);
}

static void drawCircleLetterIcon(TFT_eSprite &canvas, int16_t x, int16_t y,
                                 const char *letter, uint16_t color) {
  canvas.drawCircle(x + 10, y + 10, 10, color);
  canvas.drawCircle(x + 10, y + 10, 8, rgb(19, 49, 63));
  drawFreeText(canvas, letter, x + 5, y + 4, &FreeSans9pt7b, color);
}

static void drawSideCard(TFT_eSprite &canvas, int16_t x, int16_t y, int16_t w, int16_t h) {
  canvas.fillRoundRect(x, y, w, h, 8, C_PANEL());
  canvas.drawRoundRect(x, y, w, h, 8, rgb(30, 54, 67));
}

static void drawSideDivider(TFT_eSprite &canvas, int16_t x, int16_t y, int16_t w, int16_t h) {
  canvas.drawFastHLine(x + 20, y + h / 2, w - 40, C_DIVIDER());
}
...
static void drawLeftInfoPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  drawSideCard(canvas, x, y, LEFT_W, LEFT_H);
  drawSideDivider(canvas, x, y, LEFT_W, LEFT_H);

  char power[12];
  char delta[12];
  formatIntText(power, sizeof(power), values.power);
  formatIntText(delta, sizeof(delta), values.deltaCellVoltageMv);

  drawCompactNumberUnitRight(canvas, delta, isnan(values.deltaCellVoltageMv) ? "" : "mv",
                             x + LEFT_W - 16, y + 18, LEFT_W - 20,
                             &FreeSansBold12pt7b, C_DELTA_VALUE());
  drawCompactNumberUnitRight(canvas, power, isnan(values.power) ? "" : "w",
                             x + LEFT_W - 16, y + 63, LEFT_W - 20,
                             &FreeSansBold12pt7b, C_POWER_VALUE());
}
...
static void drawSocPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  canvas.fillRect(CENTER_X - originX, CENTER_Y - originY, CENTER_W, CENTER_H, C_BG());
  canvas.fillCircle(cx, cy, 50, rgb(5, 15, 25));
  canvas.drawCircle(cx, cy, 48, rgb(8, 23, 35));
  ...
  const int16_t textX = cx - numW / 2;
  const int16_t numY = cy - 15;
  drawFreeText(canvas, socText, textX, numY, socFont, C_TEXT());
  drawTinyUnit(canvas, "%", cx - tinyUnitWidth("%") / 2, cy + 30, C_MUTED());
  drawUiText(canvas, UI_TXT_SOC_LABEL, cx - uiTextWidth(UI_TXT_SOC_LABEL) / 2, cy + 41, C_MUTED());
}

static void drawRemainingCapacity(TFT_eSprite &canvas, int16_t originX, int16_t originY,
                                  const DashboardUiValues &values) {
  char remain[12];
  formatFloatText(remain, sizeof(remain), values.remainingCapacityAh, 1);
  const GFXfont *numberFont = &FreeSansBold9pt7b;
  const int16_t numW = freeTextWidth(canvas, remain, numberFont);
  const int16_t unitW = isnan(values.remainingCapacityAh) ? 0 : tinyUnitWidth("ah");
  const int16_t x = SOC_CX + 8 - originX - (numW + unitW + 3) / 2;
  const int16_t y = 146 - originY;
  drawFreeText(canvas, remain, x, y, numberFont, C_TEXT());
  if (!isnan(values.remainingCapacityAh)) {
    drawTinyUnit(canvas, "ah", x + numW + 3, y + 12, C_MUTED());
  }
}

static void drawCenterPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
                            const DashboardUiValues &values) {
  drawSocPanel(canvas, originX, originY, values);
  drawRemainingCapacity(canvas, originX, originY, values);
}

static void drawRightInfoPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
                               const DashboardUiValues &values) {
  const int16_t x = RIGHT_X - originX;
  const int16_t y = RIGHT_Y - originY;
  drawSideCard(canvas, x, y, RIGHT_W, RIGHT_H);
  drawSideDivider(canvas, x, y, RIGHT_W, RIGHT_H);
  ...
  drawCompactNumberUnitRight(canvas, voltage, isnan(values.voltage) ? "" : "v",
                             x + RIGHT_W - 16, y + 18, RIGHT_W - 20,
                             &FreeSansBold12pt7b, C_VOLTAGE_VALUE());
  drawCompactNumberUnitRight(canvas, current, isnan(values.current) ? "" : "a",
                             x + RIGHT_W - 16, y + 63, RIGHT_W - 20,
                             &FreeSansBold12pt7b, C_CURRENT_VALUE());
}

static void drawLoadingAnimation(TFT_eSprite &canvas, int16_t cx, int16_t cy) {
  const int16_t angle = (millis() / 5) % 360;
  drawThickArc(canvas, cx, cy, 31, 4, 0, 360, rgb(21, 43, 61));
  drawThickArc(canvas, cx, cy, 31, 4, angle, angle + 82, C_ACCENT());
  drawThickArc(canvas, cx, cy, 31, 4, angle + 138, angle + 193, C_BLUE());
  drawThickArc(canvas, cx, cy, 31, 4, angle + 252, angle + 292, rgb(92, 160, 222));
}
...
static void drawStateScreen(const char *title, const char *subtitle, uint8_t scene) {
  ...
  drawCenteredUiText(frame, title, 29, C_TEXT());

  if (scene == UI_SCENE_WAITING) {
    drawPhoneWaitAnimation(frame, 160, 86);
  } else {
    drawLoadingAnimation(frame, 160, 88);
  }
...
static void drawConnectFailedScreen(const char *reason) {
  (void) reason;
  drawStateScreen(UI_TXT_CONNECTING_BATTERY, UI_TXT_RECONNECTING_DOTS, UI_SCENE_CONNECTING);
}
...
static void drawDashboardFrame(TFT_eSprite &canvas, const DashboardUiValues &values) {
  ...
  drawCenterPanel(canvas, 0, 0, values);
  drawRightInfoPanel(canvas, 0, 0, values);
}
...
static void setDashboardTarget(const BmsData &data) {
  ...
  uiTarget.power = data.valid && !isnan(data.power) ? data.power : NAN;
  ...
  uiTarget.maxCellVoltage = NAN;
  uiTarget.minCellVoltage = NAN;
  if (!uiTargetsReady) {
    uiShown = uiTarget;
    uiTargetsReady = true;
    resetDashboardCache();
...
static void redrawDashboardRegions(bool force) {
  ...
  formatIntText(power, sizeof(power), uiShown.power);
  ...
  const bool centerChanged = force || dashboardCachedSocBar != socBar ||
                             strcmp(dashboardCachedSocText, socText) != 0 ||
  ...
  const bool leftChanged = force || strcmp(dashboardCachedPower, power) != 0 ||
                           strcmp(dashboardCachedDelta, delta) != 0;
  const bool rightChanged = force || strcmp(dashboardCachedVoltage, voltage) != 0 ||
                            strcmp(dashboardCachedCurrent, current) != 0;

  if (topChanged && beginRegion(TOP_TEMP_W, TOP_TEMP_H, C_BG())) {
    drawTopTempArea(region, uiShown, TOP_TEMP_X, TOP_TEMP_Y);
    pushRegion(TOP_TEMP_X, TOP_TEMP_Y);
  }
  if (topChanged && beginRegion(TOP_ICON_W, TOP_ICON_H, C_BG())) {
    drawTopIconArea(region, TOP_ICON_X, TOP_ICON_Y);
    pushRegion(TOP_ICON_X, TOP_ICON_Y);
  ...
  if (leftChanged && beginRegion(LEFT_W, LEFT_H, C_BG())) {
    ...
    storeCachedText(dashboardCachedPower, sizeof(dashboardCachedPower), power);
    storeCachedText(dashboardCachedDelta, sizeof(dashboardCachedDelta), delta);
  }
  if (rightChanged && beginRegion(RIGHT_W, RIGHT_H, C_BG())) {
    drawRightInfoPanel(region, RIGHT_X, RIGHT_Y, uiShown);
    pushRegion(RIGHT_X, RIGHT_Y);
  ...
  if (centerChanged && beginRegion(CENTER_W, CENTER_H, C_BG())) {
    drawCenterPanel(region, CENTER_X, CENTER_Y, uiShown);
    pushRegion(CENTER_X, CENTER_Y);
...
static void updateDashboardValues(bool force = false) {
  ...
  const uint32_t now = millis();
  if (!force && now - lastDashboardAnimMs < 120) {
    return;
  }
  ...
  changed |= approachValue(uiShown.power, uiTarget.power, 0.20f, 1.0f);
  changed |= approachValue(uiShown.deltaCellVoltageMv, uiTarget.deltaCellVoltageMv, 0.20f, 1.0f);
