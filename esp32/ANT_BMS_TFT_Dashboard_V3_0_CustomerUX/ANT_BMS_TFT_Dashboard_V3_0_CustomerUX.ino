...
static const char *FIRMWARE_VERSION = "ANT_BMS_TFT_Dashboard_V3_0_CustomerUX";
...
static const int16_t SOC_R = 47;
static const int16_t SOC_DOT_R = 4;
static const int16_t CAP_X = 218;
static const int16_t CAP_Y = 98;
static const int16_t CAP_W = 100;
static const int16_t CAP_H = 56;
...
static void drawLeftInfoPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  drawNumberWithUnitFit(canvas, voltage, isnan(values.voltage) ? "" : "V",
                        x + 13, y + 27, LEFT_W - 18, &FreeSansBold18pt7b, &FreeSansBold12pt7b,
                        C_TEXT(), C_MUTED());
  drawUiText(canvas, UI_TXT_CURRENT, x + 13, y + 59, C_ACCENT());
  drawNumberWithUnitFit(canvas, current, isnan(values.current) ? "" : "A",
                        x + 13, y + 75, LEFT_W - 18, &FreeSansBold18pt7b, &FreeSansBold12pt7b,
                        C_TEXT(), C_MUTED());
}
...
static void drawSocPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  const GFXfont *socFont = freeTextWidth(canvas, socText, &FreeSansBold24pt7b) > 62 ? &FreeSansBold18pt7b : &FreeSansBold24pt7b;
  ...
  const int16_t numY = socFont == &FreeSansBold24pt7b ? cy - 27 : cy - 20;
  drawFreeText(canvas, socText, textX, numY, socFont, C_TEXT());
  drawFreeText(canvas, "%", textX + numW + 2, cy - 10, &FreeSansBold12pt7b, C_TEXT());
  drawUiText(canvas, UI_TXT_SOC_LABEL, cx - uiTextWidth(UI_TXT_SOC_LABEL) / 2, cy + 34, C_MUTED());
}

static void drawCapacityValue(TFT_eSprite &canvas, int16_t x, int16_t y, int16_t maxWidth,
                              const char *label, const char *number, bool hasValue) {
  drawUiTextSmall(canvas, label, x, y, C_MUTED());

  const GFXfont *numberFont = &FreeSansBold9pt7b;
  int16_t numberW = freeTextWidth(canvas, number, numberFont);
  const int16_t unitW = hasValue ? freeTextWidth(canvas, "Ah", &FreeSans9pt7b) + 3 : 0;
  ...
  if (numberW + unitW <= maxWidth) {
    drawFreeText(canvas, number, x, y + 18, numberFont, C_TEXT());
    if (hasValue) {
    ...
    canvas.drawString(number, x, y + 21, 2);
    if (hasValue) {
      canvas.setTextColor(C_MUTED());
      canvas.drawString("Ah", x + canvas.textWidth(number, 2) + 2, y + 24, 1);
    }
  }
}
...
static void drawCapacityPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
                              const DashboardUiValues &values) {
  const int16_t x = CAP_X - originX;
  const int16_t y = CAP_Y - originY;
  canvas.fillRoundRect(x, y, CAP_W, CAP_H, 8, C_PANEL());
  canvas.drawRoundRect(x, y, CAP_W, CAP_H, 8, rgb(24, 43, 54));
  canvas.drawFastVLine(x + 47, y + 10, CAP_H - 20, rgb(45, 64, 73));
  ...
  drawCapacityValue(canvas, x + 9, y + 9, 38, UI_TXT_TOTAL_CAPACITY, total, !isnan(values.totalCapacityAh));
  drawCapacityValue(canvas, x + 54, y + 9, 43, UI_TXT_REMAINING_CAPACITY, remain, !isnan(values.remainingCapacityAh));
}
...
static void redrawDashboardRegions(bool force) {
  ...
  if (capacityChanged && beginRegion(CAP_W, CAP_H, C_BG())) {
    drawCapacityPanel(region, CAP_X, CAP_Y, uiShown);
    pushRegion(CAP_X, CAP_Y);
