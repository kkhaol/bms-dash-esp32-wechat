...
static const uint32_t AUTO_RETRY_INTERVAL_MS = 9000;
...
static const char *FIRMWARE_VERSION = "ANT_BMS_TFT_Dashboard_V2_5_CustomerUX";
...
static const uint8_t CAT_CONNECTING = 0;
static const uint8_t CAT_SEARCHING = 1;
static const uint8_t CAT_WAITING = 2;
...
static void drawAppMark(TFT_eSprite &canvas, int16_t originX = 0, int16_t originY = 0) {
  drawUiText(canvas, UI_TXT_APP, 18 - originX, 12 - originY, C_TEXT());
  canvas.fillRoundRect(63 - originX, 18 - originY, 22, 2, 1, C_ACCENT());
}
...
static uint16_t bmsIconColor() {
  ...
  if (appState == TRY_CONNECT_LAST_BMS || appState == TRY_CONNECT_STRONGEST_BMS ||
      appState == SCANNING_BMS || appState == SWITCHING_BMS || appState == BLE_SETUP_READY) {
...
static void drawBluetoothIcon(TFT_eSprite &canvas, int16_t x, int16_t y, uint16_t color) {
  canvas.drawLine(x + 8, y, x + 8, y + 28, color);
  canvas.drawLine(x + 8, y, x + 18, y + 9, color);
  canvas.drawLine(x + 18, y + 9, x + 8, y + 15, color);
  canvas.drawLine(x + 8, y + 13, x + 18, y + 20, color);
  canvas.drawLine(x + 18, y + 20, x + 8, y + 28, color);
  canvas.drawLine(x + 2, y + 7, x + 18, y + 20, color);
  canvas.drawLine(x + 2, y + 21, x + 18, y + 9, color);
}

static void drawPhoneIcon(TFT_eSprite &canvas, int16_t x, int16_t y, uint16_t color) {
  canvas.drawRoundRect(x, y, 15, 26, 3, color);
  canvas.drawFastHLine(x + 5, y + 3, 5, color);
  canvas.fillCircle(x + 7, y + 22, 1, color);
}

static void drawConnectionIcons(TFT_eSprite &canvas, int16_t originX = 0, int16_t originY = 0) {
  drawBluetoothIcon(canvas, 268 - originX, 10 - originY, bmsIconColor());
  drawPhoneIcon(canvas, 298 - originX, 10 - originY, phoneConnected ? C_BLUE() : C_DIM());
}

static void drawThermometerIcon(TFT_eSprite &canvas, int16_t x, int16_t y, uint16_t color) {
  canvas.drawRoundRect(x + 5, y, 6, 20, 3, color);
  canvas.fillCircle(x + 8, y + 22, 6, color);
  canvas.fillRoundRect(x + 7, y + 5, 2, 14, 1, color);
  canvas.fillCircle(x + 8, y + 22, 3, C_BG());
}

static void drawTempValue(TFT_eSprite &canvas, const DashboardUiValues &values,
                          int16_t originX = 0, int16_t originY = 0) {
  drawThermometerIcon(canvas, 18 - originX, 10 - originY, C_BLUE());
  char temp[12];
  formatIntText(temp, sizeof(temp), values.mosTemp);
  drawFreeText(canvas, temp, 39 - originX, 12 - originY, &FreeSansBold12pt7b, C_TEXT());
  const int16_t numW = freeTextWidth(canvas, temp, &FreeSansBold12pt7b);
  if (!isnan(values.mosTemp)) {
    canvas.drawCircle(42 - originX + numW, 17 - originY, 2, C_MUTED());
    drawFreeText(canvas, "C", 47 - originX + numW, 12 - originY, &FreeSans9pt7b, C_MUTED());
  }
}
...
static void drawNumberWithUnit(TFT_eSprite &canvas, const char *number, const char *unit,
                               int16_t x, int16_t y, const GFXfont *numberFont,
                               const GFXfont *unitFont, uint16_t numberColor, uint16_t unitColor) {
  drawFreeText(canvas, number, x, y, numberFont, numberColor);
  const int16_t numberW = freeTextWidth(canvas, number, numberFont);
  if (unit[0]) {
    drawFreeText(canvas, unit, x + numberW + 4, y + 15, unitFont, unitColor);
  }
}

static void drawLeftInfoPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
                              const DashboardUiValues &values) {
  const int16_t x = 14 - originX;
  const int16_t y = 49 - originY;
  canvas.fillRoundRect(x, y, 104, 108, 9, C_PANEL());
  canvas.drawRoundRect(x, y, 104, 108, 9, rgb(28, 48, 59));
  canvas.drawFastHLine(x + 16, y + 55, 72, rgb(45, 64, 73));

  char voltage[12];
  char current[12];
  formatFloatText(voltage, sizeof(voltage), values.voltage, 1);
  formatFloatText(current, sizeof(current), values.current, 1);
  drawUiText(canvas, UI_TXT_VOLTAGE, x + 18, y + 14, C_ACCENT());
  drawNumberWithUnit(canvas, voltage, isnan(values.voltage) ? "" : "V",
                     x + 18, y + 31, &FreeSansBold18pt7b, &FreeSans9pt7b, C_TEXT(), C_MUTED());
  drawUiText(canvas, UI_TXT_CURRENT, x + 18, y + 68, C_ACCENT());
  drawNumberWithUnit(canvas, current, isnan(values.current) ? "" : "A",
                     x + 18, y + 83, &FreeSansBold18pt7b, &FreeSans9pt7b, C_TEXT(), C_MUTED());
}

static void drawSocPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
                         const DashboardUiValues &values) {
  const int16_t cx = 166 - originX;
  const int16_t cy = 95 - originY;
  const int soc = isnan(values.soc) ? -1 : constrain(int(roundf(values.soc)), 0, 100);

  canvas.fillRect(116 - originX, 38 - originY, 102, 122, C_BG());
  canvas.fillCircle(cx, cy, 56, rgb(8, 21, 31));
  drawThickArc(canvas, cx, cy, 48, 4, -140, 140, rgb(20, 39, 51));
  if (soc >= 0) {
    drawThickArc(canvas, cx, cy, 48, 4, -140, -140 + int(soc * 2.8f), getSocColor(soc));
  }
  ...
  const int16_t textX = cx - (numW + pctW + 2) / 2;
  drawFreeText(canvas, socText, textX, cy - 26, &FreeSansBold24pt7b, C_TEXT());
  drawFreeText(canvas, "%", textX + numW + 2, cy - 11, &FreeSansBold12pt7b, C_TEXT());
  drawUiText(canvas, UI_TXT_SOC_LABEL, cx - uiTextWidth(UI_TXT_SOC_LABEL) / 2, cy + 35, C_MUTED());
}

static void drawCapacityValue(TFT_eSprite &canvas, int16_t x, int16_t y, const char *label,
                              const char *number, bool hasValue) {
  drawUiText(canvas, label, x, y, C_MUTED());
  drawFreeText(canvas, number, x, y + 18, &FreeSansBold9pt7b, C_TEXT());
  const int16_t numberW = freeTextWidth(canvas, number, &FreeSansBold9pt7b);
  if (hasValue) {
    drawFreeText(canvas, "Ah", x + numberW + 3, y + 22, &FreeSans9pt7b, C_MUTED());
  }
}

static void drawCapacityPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
                              const DashboardUiValues &values) {
  const int16_t x = 224 - originX;
  const int16_t y = 82 - originY;
  canvas.fillRoundRect(x, y, 88, 76, 8, C_PANEL());
  canvas.drawRoundRect(x, y, 88, 76, 8, rgb(28, 48, 59));
  canvas.drawFastHLine(x + 10, y + 38, 68, rgb(45, 64, 73));
  ...
  drawCapacityValue(canvas, x + 10, y + 7, UI_TXT_TOTAL_CAPACITY, total, !isnan(values.totalCapacityAh));
  drawCapacityValue(canvas, x + 10, y + 43, UI_TXT_REMAINING_CAPACITY, remain, !isnan(values.remainingCapacityAh));
}

static void drawCatFace(TFT_eSprite &canvas, int16_t x, int16_t y, uint16_t fill, uint16_t line) {
  canvas.fillCircle(x, y, 13, fill);
  canvas.fillTriangle(x - 9, y - 8, x - 4, y - 20, x + 1, y - 8, fill);
  canvas.fillTriangle(x + 7, y - 8, x + 12, y - 20, x + 17, y - 8, fill);
  canvas.drawTriangle(x - 9, y - 8, x - 4, y - 20, x + 1, y - 8, line);
  canvas.drawTriangle(x + 7, y - 8, x + 12, y - 20, x + 17, y - 8, line);
  canvas.fillCircle(x - 5, y - 2, 2, TFT_BLACK);
  canvas.fillCircle(x + 6, y - 2, 2, TFT_BLACK);
  canvas.drawPixel(x, y + 3, TFT_BLACK);
  canvas.drawLine(x, y + 4, x - 4, y + 7, TFT_BLACK);
  canvas.drawLine(x, y + 4, x + 4, y + 7, TFT_BLACK);
}

static void drawRunningCat(TFT_eSprite &canvas, int16_t x, int16_t y, uint8_t phase) {
  const int8_t leg = (phase % 2) ? 5 : -3;
  const uint16_t body = rgb(156, 166, 172);
  const uint16_t line = rgb(88, 104, 112);
  canvas.fillRoundRect(x - 34, y - 5, 42, 18, 9, body);
  canvas.drawRoundRect(x - 34, y - 5, 42, 18, 9, line);
  drawCatFace(canvas, x + 16, y - 8, rgb(210, 216, 218), line);
  canvas.drawLine(x - 34, y + 1, x - 47, y - 9, line);
  canvas.drawLine(x - 47, y - 9, x - 51, y - 3, line);
  canvas.drawLine(x - 24, y + 11, x - 34, y + 19 + leg, line);
  canvas.drawLine(x - 11, y + 12, x - 2, y + 20 - leg, line);
  canvas.drawLine(x - 2, y + 11, x + 9, y + 18 + leg, line);
  canvas.fillCircle(x - 44, y + 23, 1, C_DIM());
  canvas.fillCircle(x - 32, y + 24, 1, C_DIM());
}

static void drawSearchingCat(TFT_eSprite &canvas, int16_t x, int16_t y, uint8_t phase) {
  drawRunningCat(canvas, x, y, phase);
  canvas.drawCircle(x + 49, y - 7, 13, C_ACCENT());
  canvas.drawLine(x + 58, y + 3, x + 71, y + 16, C_ACCENT());
  const int16_t radarX = x + 88;
  const int16_t radarY = y - 7;
  canvas.drawCircle(radarX, radarY, 4 + (phase % 4) * 4, rgb(22, 58, 70));
  canvas.fillCircle(radarX, radarY, 3, C_BLUE());
}

static void drawWaitingCat(TFT_eSprite &canvas, int16_t x, int16_t y, uint8_t phase) {
  const uint16_t body = rgb(156, 166, 172);
  const uint16_t line = rgb(88, 104, 112);
  canvas.fillRoundRect(x - 15, y - 4, 30, 42, 15, body);
  canvas.drawRoundRect(x - 15, y - 4, 30, 42, 15, line);
  drawCatFace(canvas, x, y - 10, rgb(216, 220, 222), line);
  canvas.drawLine(x - 15, y + 28, x - 26, y + 36, line);
  canvas.drawLine(x + 15, y + 28, x + 26, y + 36, line);
  if ((phase % 6) < 3) {
    canvas.drawLine(x - 26, y + 2, x - 37, y - 4, line);
  }
  drawPhoneIcon(canvas, x + 62, y - 24, rgb(70, 90, 102));
}
...
static void drawStateScreen(const char *title, const char *subtitle, uint8_t scene) {
  ...
  const uint8_t phase = (millis() / STATE_ANIM_INTERVAL_MS) % 12;
  frame.fillSprite(C_BG());
  drawAppMark(frame);
  drawConnectionIcons(frame);
  frame.fillRoundRect(12, 37, 296, 118, 10, C_PANEL());
  frame.drawRoundRect(12, 37, 296, 118, 10, rgb(26, 48, 59));
  drawCenteredUiText(frame, title, 51, C_TEXT());

  if (scene == CAT_CONNECTING) {
    drawRunningCat(frame, 145 + (phase % 3), 94, phase);
    for (int i = 0; i < 3; i++) {
      const bool on = ((phase / 2) + i) % 3 == 0;
      frame.fillCircle(219 + i * 11, 93, 3, on ? C_ACCENT() : C_DIM());
    }
  } else if (scene == CAT_SEARCHING) {
    drawSearchingCat(frame, 121, 96, phase);
  } else {
    drawWaitingCat(frame, 160, 94, phase);
  }

  drawCenteredUiText(frame, subtitle, 135, C_MUTED());
...
static void drawBootAnimation() {
  ...
  while (millis() - start < 900) {
    drawStateScreen(UI_TXT_BOOTING, UI_TXT_WAIT, CAT_WAITING);
    delay(STATE_ANIM_INTERVAL_MS);
  }
}
...
static void drawBleReadyScreen() {
  drawStateScreen(UI_TXT_CONNECTING_BATTERY, UI_TXT_WAIT, CAT_CONNECTING);
}

static void drawWaitConfigScreen(const char *reason) {
  (void) reason;
  drawStateScreen(UI_TXT_WAIT_PHONE, UI_TXT_PICK_ON_PHONE, CAT_WAITING);
}

static void drawTryLastBmsScreen() {
  drawStateScreen(UI_TXT_CONNECTING_BATTERY, UI_TXT_CONNECT_LAST, CAT_CONNECTING);
}

static void drawTryStrongestBmsScreen() {
  drawStateScreen(UI_TXT_CONNECTING_BATTERY, UI_TXT_CONNECT_NEARBY, CAT_CONNECTING);
}

static void drawManualBmsScanScreen() {
  drawStateScreen(UI_TXT_SEARCHING_BATTERY, UI_TXT_PICK_ON_PHONE, CAT_SEARCHING);
}

static void drawSwitchingBmsScreen(const String &mac) {
  (void) mac;
  drawStateScreen(UI_TXT_SWITCHING_BATTERY, UI_TXT_WAIT, CAT_CONNECTING);
}

static void drawConnectFailedScreen(const char *reason) {
  (void) reason;
  drawStateScreen(UI_TXT_CONNECTING_BATTERY, UI_TXT_RECONNECTING, CAT_CONNECTING);
}
...
static void drawConnectingScreen(const String &deviceName, int rssi) {
  (void) deviceName;
  (void) rssi;
  drawStateScreen(UI_TXT_CONNECTING_BATTERY, connectingSubtitle(), CAT_CONNECTING);
}

static void drawMessageScreen(const char *line1, const char *line2) {
  drawStateScreen(line1, line2, CAT_WAITING);
}
...
static void drawDashboardFrame(TFT_eSprite &canvas, const DashboardUiValues &values) {
  ...
  drawCapacityPanel(canvas, 0, 0, values);
}
...
static void redrawDashboardRegions(bool force) {
  ...
  const bool capacityChanged = force || strcmp(dashboardCachedTotalCapacity, totalCapacity) != 0 ||
  ...
  if (leftChanged && beginRegion(110, 114, C_BG())) {
    drawLeftInfoPanel(region, 12, 45, uiShown);
    pushRegion(12, 45);
  ...
  if (socChanged && beginRegion(116, 126, C_BG())) {
    drawSocPanel(region, 108, 36, uiShown);
    pushRegion(108, 36);
    dashboardCachedSocBar = socBar;
    storeCachedText(dashboardCachedSocText, sizeof(dashboardCachedSocText), socText);
  }
  if (capacityChanged && beginRegion(94, 82, C_BG())) {
    drawCapacityPanel(region, 221, 79, uiShown);
    pushRegion(221, 79);
...
static BmsCandidate scanForBms(const String &preferredMac) {
  ...
  while (millis() - scanStartMs < BMS_SCAN_MS) {
    const uint32_t elapsed = millis() - scanStartMs;
    const uint32_t remaining = elapsed < BMS_SCAN_MS ? BMS_SCAN_MS - elapsed : 0;
    const uint32_t chunkMs = remaining > 500 ? 500 : remaining;
    if (chunkMs == 0) {
      break;
    }
    scan->getResults(chunkMs, false);
    refreshStateAnimation(true);
    if (preferredScanMac.length() > 0 && bestCandidate.found &&
        bestCandidate.mac.equalsIgnoreCase(preferredScanMac)) {
      break;
    }
  }
...
static void handleAutoRetry() {
  ...
  if (appState == SCANNING_BMS || appState == SWITCHING_BMS ||
      appState == TRY_CONNECT_LAST_BMS || appState == TRY_CONNECT_STRONGEST_BMS) {
    return;
  }
  ...
  autoRetryPending = false;
  Serial.println("[POLICY] Auto retry begin: saved BMS first, then strongest nearby");
