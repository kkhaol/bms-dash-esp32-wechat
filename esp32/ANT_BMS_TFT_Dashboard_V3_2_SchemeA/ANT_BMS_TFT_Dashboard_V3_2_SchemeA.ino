...
static const char *FIRMWARE_VERSION = "ANT_BMS_TFT_Dashboard_V3_2_SchemeA";
...
static const int16_t LEFT_X = 24;
static const int16_t LEFT_Y = 48;
static const int16_t LEFT_W = 76;
static const int16_t LEFT_H = 92;
static const int16_t SOC_CX = 160;
static const int16_t SOC_CY = 83;
static const int16_t SOC_R = 46;
static const int16_t SOC_DOT_R = 4;
static const int16_t CENTER_X = 101;
static const int16_t CENTER_Y = 31;
static const int16_t CENTER_W = 118;
static const int16_t CENTER_H = 132;
static const int16_t RIGHT_X = 220;
static const int16_t RIGHT_Y = 48;
static const int16_t RIGHT_W = 76;
static const int16_t RIGHT_H = 92;
...
static void drawCompactNumberUnit(TFT_eSprite &canvas, const char *number, const char *unit,
  ...
  const int16_t unitW = unit[0] ? freeTextWidth(canvas, unit, &FreeSans9pt7b) + 4 : 0;
  ...
  if (unit[0]) {
    const int16_t unitY = numberFont == preferredFont ? y + 13 : y + 7;
    drawFreeText(canvas, unit, x + numberW + 4, unitY, &FreeSans9pt7b, C_MUTED());
  }
}
...
static void drawLeftInfoPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  drawPowerIcon(canvas, x + 17, y + 14, C_ACCENT());
  drawCompactNumberUnit(canvas, power, isnan(values.power) ? "" : "w",
                        x + 17, y + 42, LEFT_W - 22, &FreeSansBold12pt7b, C_TEXT());
  drawDeltaIcon(canvas, x + 16, y + 57, C_ACCENT());
  drawCompactNumberUnit(canvas, delta, isnan(values.deltaCellVoltageMv) ? "" : "mv",
                        x + 17, y + 71, LEFT_W - 22, &FreeSansBold12pt7b, C_TEXT());
}
...
static void drawSocPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  canvas.fillCircle(cx, cy, 58, rgb(5, 15, 25));
  canvas.drawCircle(cx, cy, 55, rgb(8, 23, 35));
  ...
  const int16_t numY = cy - 10;
  drawFreeText(canvas, socText, textX, numY, socFont, C_TEXT());
  drawFreeText(canvas, "%", textX + numW + 3, cy + 7, &FreeSans9pt7b, C_TEXT());
  drawUiText(canvas, UI_TXT_SOC_LABEL, cx - uiTextWidth(UI_TXT_SOC_LABEL) / 2, cy + 38, C_MUTED());
}
...
static void drawRemainingCapacity(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  const GFXfont *numberFont = &FreeSansBold12pt7b;
  const int16_t numW = freeTextWidth(canvas, remain, numberFont);
  const int16_t unitW = isnan(values.remainingCapacityAh) ? 0 : freeTextWidth(canvas, "ah", &FreeSans9pt7b);
  const int16_t x = SOC_CX - originX - (numW + unitW + 5) / 2;
  const int16_t y = 136 - originY;
  drawFreeText(canvas, remain, x, y, numberFont, C_TEXT());
  if (!isnan(values.remainingCapacityAh)) {
    drawFreeText(canvas, "ah", x + numW + 5, y + 9, &FreeSans9pt7b, C_MUTED());
  }
}
...
static void drawRightInfoPanel(TFT_eSprite &canvas, int16_t originX, int16_t originY,
  ...
  drawCircleLetterIcon(canvas, x + 16, y + 13, "v", C_ACCENT());
  drawCompactNumberUnit(canvas, voltage, isnan(values.voltage) ? "" : "v",
                        x + 16, y + 39, RIGHT_W - 20, &FreeSansBold12pt7b, C_TEXT());
  drawCircleLetterIcon(canvas, x + 16, y + 56, "a", C_ACCENT());
  drawCompactNumberUnit(canvas, current, isnan(values.current) ? "" : "a",
                        x + 16, y + 70, RIGHT_W - 20, &FreeSansBold12pt7b, C_TEXT());
}
...
static void drawStateScreen(const char *title, const char *subtitle, uint8_t scene) {
  ...
  drawCenteredUiText(frame, title, 54, C_TEXT());

  if (scene == UI_SCENE_WAITING) {
    drawPhoneWaitAnimation(frame, 160, 92);
  } else {
    drawLoadingAnimation(frame, 160, 99);
  }

  drawCenteredUiText(frame, subtitle, 141, C_MUTED());
